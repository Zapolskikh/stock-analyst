...
def score_valuation(nd: NormalisedData, bm: Benchmark) -> BlockScore:
    ...
    # --- P/S ratio (market_cap / revenue) — шкала из бенчмарка ------------
    last_rev = _last_valid(nd.revenue_annual)
    ...
    # --- FCF yield (FCF / market_cap × 100) — шкала из бенчмарка ----------
    last_fcf = _last_valid(nd.fcf_annual)
    ...
    # Осмысленен только для прибыльных растущих компаний.
    # Пропускается если: eps_growth ≤ 0, pe_forward отсутствует, нет шкалы в бенчмарке.
    if pe_f is not None and math.isfinite(pe_f) and pe_f > 0:
    ...
    # EBITDA = last ebitda_annual (operating_income + D&A, pre-computed in normalizer)
    ...
    if (nd.market_cap and math.isfinite(nd.market_cap)
            and math.isfinite(last_ltd) and math.isfinite(last_ebitda)
            and last_ebitda > 0):
