...
[Старт]
   ↓
✅ [Получение данных по акции]        ← src/data/price.py, src/data/sec_edgar.py, src/data/cache.py
   ↓
✅ [Очистка и нормализация данных]    ← src/data/normalizer.py  (реализовано)
   ↓
✅ [Определение типа компании]        ← src/classifier.py        (реализовано)
   ├─> Hypergrowth Tech
   ├─> Mature Tech
   ├─> Pharma / Healthcare
   ├─> Dividend / Defensive
   ...
   └─> Other
   ↓
✅ [Выбор эталонной модели для этого типа]  ← src/models/benchmarks.py  (реализовано)
   ↓
✅ [Расчет 5 блоков оценки]           ← src/scoring/*             (реализовано)
   ├─> Quality
   ├─> Valuation
   ├─> Technical State
   ├─> Risk
   └─> Style Fit
   ↓
✅ [Расчет score по 3 горизонтам]     ← src/engine/engine.py      (реализовано)
   ├─> Short-term
   ├─> Medium-term
   └─> Long-term
   ↓
✅ [Проверка стоп-факторов]           ← src/engine/engine.py      (реализовано)
   ├─> Слишком дорогая?
   ...
   ↓
✅ [Формирование итогового рейтинга]  ← src/engine/engine.py      (реализовано)
   ├─> 85–100 = Strong Candidate
   ├─> 70–84 = Good / Watchlist
   ├─> 55–69 = Mixed
   └─> <55 = Weak
   ↓
✅ [Финальный вывод]                  ← src/output/formatter.py   (реализовано)
   ├─> Buy now
   ├─> Watch
   ├─> Hold
   └─> Avoid
...
> **Реализовано:** `src/data/price.py` (OHLCV, дивиденды, splits, yfinance info) · `src/data/sec_edgar.py` (SEC EDGAR XBRL: revenue, EPS, FCF, debt, margins) · `src/data/cache.py` (TTL-кеш) · `fetch.py` (CLI) · `src/charts/` (6 фундаментальных чартов + price chart)
...
> **Реализовано:** `src/data/normalizer.py` — `normalise(fundamentals, price_df, info)` → `NormalisedData`: выровненные годовые ряды, FCF, margins, growth rates, ROE/ROA, D/E, scalars из yfinance, data_quality оценка.
...
> **Реализовано:** `src/classifier.py` — `classify(nd)` → `ClassificationResult`: 7 типов (Hypergrowth Tech, Mature Tech, Pharma, Dividend/Defensive, Cyclical, Financial, Turnaround) + OTHER. Rule-based scoring по сигналам: revenue growth, margins, sector, dividend yield, R&D ratio, debt leverage, убыточность.
...
> **Реализовано:** `src/models/benchmarks.py` — `get_benchmark(CompanyType)` → `Benchmark`: веса 5 блоков + пороговые шкалы (`Threshold` с линейной интерполяцией) для каждой метрики, настроенные под каждый тип компании.
...
> **Реализовано.** `src/scoring/quality.py`, `valuation.py`, `technical.py`, `risk.py`, `style_fit.py` + `base.py`. 52 теста.

## ✅ Блок A. Business Quality

> `src/scoring/quality.py`
...
> **Реализовано.** `src/engine/engine.py` — `HorizonScores(short, medium, long)`, разные веса блоков на каждый горизонт.
...
> **Реализовано.** `src/engine/engine.py` — `_check_stop_factors()`: negative FCF, extreme P/E, high D/E, deteriorating margins, technical breakdown. severity: warning / critical.
...
> **Реализовано.** `src/engine/engine.py` — взвешенная сумма 5 блоков по весам бенчмарка × 10 → 0–100.
...
> **Реализовано.** `src/output/formatter.py` — `format_report()` / `format_brief()`.
...
> **Реализовано.** `src/output/formatter.py` — полный отчёт с блоками, горизонтами, стоп-факторами, analyst notes и финальным решением.
